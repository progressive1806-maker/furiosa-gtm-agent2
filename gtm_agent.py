...
def main():
    ...
    for q in dynamic_queries:
        ...
        try:
            res = requests.get(f"https://openapi.naver.com/v1/search/news.json?query={q}&display=3", headers=headers).json()
    ...
    if NARAJANGTEO_API_KEY:
        ...
        try:
            ...
            params = {'serviceKey': NARAJANGTEO_API_KEY, 'numOfRows': '15', 'inqryBgnDt': from_date, 'inqryEndDt': to_date, 'bidNtceNm': search_keyword, 'type': 'json'}
            res = requests.get(url, params=params, timeout=12).json()
            for item in res.get('response', {}).get('body', {}).get('items', []): 
                b2g_raw_data += f"공고명: {item['bidNtceNm']} / 발주처: {item['ntceInsttNm']} / 링크: {item['bidNtceDetailUrl']}\n"
        except Exception as e: 
...
- 리포트에서 '제외 기사' 같은 건 빼도 된다. **보는 리포트를 보는 사람 입장에서 필요한 내용만 들어가면 된다.**
...
"""
    ...
    prompt_v1 = (
        f"{SUPERVISOR_PROMPT}\n\n"
        f"[실시간 연동 데이터]\n"
        f"- 퓨리오사 정보: {furiosa_context}\n"
        f"- 시장 뉴스 전문(링크+본문): {market_raw_data}\n"
        f"- 실제 Jira 파이프라인 내역: {jira_context}\n\n"
        f"**[🚨 최고 중요 지시사항]**\n"
        f"1. **모델 환각 절대 금지**: 제공된 '기사본문' 텍스트를 정독해라. 기사 본문에 특정 AI 모델명(버전 포함)이 명시적으로 나오지 않는다면 절대 지어내지 말고, 빈칸으로 두거나 '직접 언급 없음'으로 표기해라.\n"
        f"2. 컨택 가능성 랭킹 최대 7개 후보 도출.\n"
        f"3. 기사 원본 링크 삽입 필수 (<a> 태그 사용).\n"
        f"4. 링크드인 검색 주소 삽입 필수: https://www.linkedin.com/search/results/people/?keywords=기업명+AI\n"
        f"5. 실제 Jira 파이프라인 전체 내역을 보고 기존 접점 여부를 정확히 판단할 것.\n"
        f"6. B2G 데이터 제외, HTML <body> 내부 형태(인라인 스타일 포함)로 작성."
    )
    ...
    prompt_v2 = (
        ...
        f"- 실제 Jira 파이프라인 내역: {jira_context}\n\n"
        f"**[🚨 최고 중요 지시사항]**\n"
        f"1. **모델 환각 절대 금지**: 제공된 '기사본문'을 정독하여 정확한 모델명(버전 포함)만 적어라. 없다면 지어내지 마라.\n"
        f"2. B2B(최대 7개) + B2G(최대 5개) 합쳐서 **총 최대 12개**의 타겟 랭킹 작성.\n"
        f"3. B2G 사업은 '발주처'가 아닌 대형 'SI 사업자'를 영업 타겟으로 명시.\n"
        f"4. 기사/공고 원본 링크 필수 삽입.\n"
        f"5. 링크드인 검색 주소 필수 삽입.\n"
        f"6. 실제 Jira 전체 내역을 보고 기존 접점 확인.\n"
        f"7. HTML <body> 내부 형태로 작성."
    )
    ...
    html_template_start = """<!DOCTYPE html><html lang="ko"><head><meta charset="utf-8"><style>body { font-family: 'Malgun Gothic', sans-serif; padding: 40px; background-color: #f4f6f9; color: #333; line-height: 1.6; } .container { max-width: 1000px; margin: 0 auto; background: white; padding: 50px; border-radius: 12px; box-shadow: 0 4px 20px rgba(0,0,0,0.05); } h1, h2, h3 { color: #004085; } a { color: #007bff; text-decoration: none; word-break: break-all; } a:hover { text-decoration: underline; } table { width: 100%; border-collapse: collapse; margin-bottom: 30px; } th, td { border: 1px solid #ddd; padding: 15px; text-align: left; } th { background-color: #f8f9fa; font-weight: bold; }</style></head><body><div class="container">"""
    ...
    with open("reports/b2b.html", "w", encoding="utf-8") as f: f.write(html_template_start + report_v1 + html_template_end)
    with open("reports/b2b_b2g.html", "w", encoding="utf-8") as f: f.write(html_template_start + report_v2 + html_template_end)
    print("✅ 성공: 본문 크롤링 및 실제 지라 전체 연동 기반 리포트 생성 완료!")
